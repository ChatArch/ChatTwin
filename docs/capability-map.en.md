...
Use this page to check which first-class capabilities `ChatTwin` currently owns, which ones are verified, and what remains out of scope for this package.
...
- **CLI Entry**
    ...
    `chattwin --help`, `chattwin --version`, `chattwin --tree`, and `chattwin --tree-brief` are the default verification entry points.

...
This page is the compact capability map for the `ChatTwin` CLI. Use it to review which commands are first-class entries and which are still boundary or planned slots. After scaffolding, update it with the real command tree; do not present unimplemented commands as available operations.
...
```text
chattwin                  # ChatTwin command-line entry
...
```text
chattwin --help           # Verify the command is installed and inspect the current command tree
chattwin --version        # Verify the installed version
chattwin --tree           # Read back the CLI contract with parameter signatures
chattwin --tree-brief     # Read back command nodes and descriptions only
```
...
```text
chattwin <group>          # Command group named after real package capability
